...
function MakeClickSort(params) {
    ...
    function jsSort(objList, byProperty, orderDir) {
    ...
    function addTableBody(table, list, sortOrderPropName, orderDir) {
        ...
        jsSort(list, sortOrderPropName, orderDir);
    ...
    function addFilterTableBody(table, objList, filterValue) {
    ...
    heading.innerHTML = params.theTitle;
    container.appendChild(heading);
        ...
        img.onclick = function () {
            ...
            window.location.hash = params.theUrl;
        };
    ...
    for (var propName in obj) {
        ...
        if (propName[0] !== "_") {
            headingText = "<img src='" + params.theSortIcon + "'/> " + headingText;
            headingCell.sortPropName = propName;
            var orderDir = true;
            headingCell.onclick = function () {
                orderDir = !orderDir;
                console.log("WILL SORT ON " + this.sortPropName);
                addTableBody(newTable, params.theObjList, this.sortPropName, orderDir);
            };
    ...
    searchInput.onkeyup = function () {
        ...
        addFilterTableBody(newTable, params.theObjList, searchInput.value);
    };
    
    // After sorting objList by sortOrderPropName, create or replace the tbody 
    // populated with data from the sorted objList.
    addTableBody(newTable, params.theObjList, params.theSortOrderPropName);
